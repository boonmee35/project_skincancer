...
function Posts() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(`${apiUrl}forum/posts/reported`);
    ...
    try {
      const fetchReport = async () => {
        const response = await axios.get(
          `${apiUrl}forum/posts/reported/post/${post.post_id}`
        );
  ...
  const updateStatus = async (status) => {
    ...
    //   text: "คุณต้องการจัดการเฉพาะรายงานนี้หรือทั้งหมดของโพสต์นี้?",
    //   icon: "question",
    //   showDenyButton: true,
    ...
    Swal.fire({
      ...
      text: "คุณต้องการจัดการรายงานโพสต์นี้หรือไม่?",
      icon: "warning",
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        try {
          await axios.put(
            `${apiUrl}forum/posts/report/${selectedPost.post_id}/action`,
            {
              action: status,
              scope,
          ...
          Swal.fire({
            icon: "success",
            title: "อัปเดตสถานะโพสต์เรียบร้อยแล้ว",
          ...
          Swal.fire({
            icon: "error",
            title: "เกิดข้อผิดพลาดในการอัปเดตสถานะโพสต์",
            text: error.response?.data?.error || "ไม่สามารถอัปเดตสถานะโพสต์ได้",
          });
        }
      }
    });
  ...
  const currentReport = posts.slice(
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-6xl mx-auto bg-white shadow rounded-lg p-6">
        <h1 className="text-2xl font-bold mb-4">โพสต์ที่ถูกแจ้งรายงาน</h1>
          ...
          <table className="min-w-full table-auto border-b">
            <thead className="bg-blue-50 text-gray-700">
              <tr>
                ...
                <th className="p-3 text-left">เนื้อหาโพสต์</th>
              ...
              {currentReport.length === 0 ? (
                ...
                currentReport.map((post, index) => (
                  <tr key={index} className="border-t hover:bg-gray-50">
                    ...
                    <td className="p-3 text-center">{post.report_count}</td>
      ...
      {isModalOpen && selectedPost && (
        <div className="fixed inset-0 bg-[#191919]/50 flex justify-center items-center z-50">
          ...
          >
            ...
            <h2 className="text-xl font-bold mb-4">รายละเอียดโพสต์</h2>
            <p className="mb-2">
              <strong>หัวข้อโพสต์:</strong> {selectedPost.title}
            </p>
            <div className="mb-4">
            ...
            <h3 className="text-lg font-semibold mb-2">รายการรายงาน</h3>
            {reports.length === 0 ? (
              <p className="text-gray-500">ไม่มีรายงาน</p>
            ) : (
